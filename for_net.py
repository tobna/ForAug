...
class ForNet(Dataset):
    ...
    def __init__(
        ...
        assert os.path.exists(
            f"{root}/fg_bg_ratios_{'train' if train else 'val'}.json"
        ...
        if background_combination == "same":
            for cls_code in classes:
                if cls_code not in self.cls_to_allowed_bg or len(self.cls_to_allowed_bg[cls_code]) == 0:
                    ...
                    print(f"Warning: no background for class {cls_code}, using {backup_backgrounds[cls_code]}")
    ...
    def __getitem__(self, idx):
        ...
        # print(f"fg_bg_ratio={orig_fg_ratio}")
        fg_scale = (
            np.random.uniform(
